...
Freelancer-Felder wiederholen einen Cube nicht komplett identisch. Reale Felder wie `br03_keswick_ice_asteroid_field.ini` enthalten in `[Cube]` zusätzlich `xaxis_rotation`, `yaxis_rotation` und `zaxis_rotation`. Der Field Creator parsed diese Listen, schreibt sie beim Generieren wieder aus und nutzt sie in der 3D-Preview deterministisch pro angezeigtem Cube, damit wiederholte Cubes rotiert wirken und nicht als starres Kachelmuster auffallen. Wichtig: Die Preview kombiniert X/Y/Z nicht frei, sondern nimmt Rotation-Tupel mit gleichem Index aus den drei Listen. Das ist näher an der beobachteten Freelancer-Darstellung, bei der längliche Asteroiden ihre Grundausrichtung behalten und Cubes eher "gerollt" bzw. geflippt wirken. Ab mehr als einem angezeigten Preview-Cube wird außerdem `empty_cube_frequency` über einen stabilen Hash aus Template-Dateiname und Cube-Koordinate simuliert. Die genaue Engine-Verteilung ist weiterhin eine Annäherung; die sichtbare Vorschau soll aber die wichtigsten Musterbrecher aus den Freelancer-INI-Daten berücksichtigen.
